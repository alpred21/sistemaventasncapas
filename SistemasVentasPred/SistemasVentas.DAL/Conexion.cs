...
namespace SistemasVentas.DAL
{
    public class conexion
    {
        ...
        {
            get { return @"Data Source=PCA-33; Initial Catalog=TIENDABD; Integrated Security=True; TrustServerCertificate=true;"; }
        ...
        public static bool VerificarUsuario2(string usuario, string contraseña)
        {
            string consulta = "SELECT COUNT(1) FROM Usuario AS U " +
                              "INNER JOIN UsuarioRol AS UR ON U.IDUsuario = UR.IDUsuario " +
                              "WHERE U.NombreUser = @Usuario AND U.Contraseña = @Contraseña " +
        ...
        public static bool VerificarUsuario3(string usuario, string contraseña)
        {
            string consulta = "SELECT COUNT(1) FROM Usuario AS U " +
                              "INNER JOIN UsuarioRol AS UR ON U.IDUsuario = UR.IDUsuario " +
                              "WHERE U.NombreUser = @Usuario AND U.Contraseña = @Contraseña " +
